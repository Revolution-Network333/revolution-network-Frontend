...
Le backend supporte plusieurs modes d'extraction web:

- **auto** (par défaut): Fallback intelligent - tente HTML, puis Browser, puis AI si nécessaire
- **html**: Extraction HTTP classique avec headers navigateur et décompression (gzip, brotli, deflate)
- **browser**: Extraction via Puppeteer (headless) pour contourner les protections anti-bot
- **css**: Extraction basée sur sélecteurs CSS personnalisés
- **ai**: Extraction purement basée sur l'IA
- **hybrid**: Combine CSS et AI pour une extraction robuste

### Fonctionnalités E-commerce

- **Détection automatique de plateformes**: Amazon, Shopify, WooCommerce
- **Pagination intelligente**: Détection automatique des liens "Suivant"
- **Standardisation de sortie**: Format unifié pour les produits (name, price, availability, etc.)
- **Sélecteurs optimisés**: Sélecteurs spécifiques par plateforme pour une extraction précise

### Robustesse

- **Système de retry**: Tentatives automatiques en cas d'échec
- **Normalisation d'URL**: Ajout automatique de https:// si manquant
- **Headers navigateur**: User-Agent et headers réalistes pour éviter les blocages
- **Décompression automatique**: Support gzip, brotli, deflate

### Stockage Cloudflare R2

Les résultats sont stockés sur Cloudflare R2 (S3-compatible):

- Configuration via variables d'environnement:
  - `R2_ENDPOINT`: URL du endpoint R2
  - `R2_ACCESS_KEY_ID`: Clé d'accès
  - `R2_SECRET_ACCESS_KEY`: Secret
  - `R2_BUCKET_NAME`: Nom du bucket
- Fallback automatique sur stockage base de données si R2 non configuré
...
```bash
GET /api/enterprise/v1/jobs/{job_id}/result
x-api-key: YOUR_API_KEY
```
